...
public final class SettingUtils {
    ...
    private static Method findApplyMethod() {
        try {
            ...
            if (DEBUG) {
                L.d(TAG, "Failed to retrieve Editor.apply(); probably doesn't exist on this phone's OS.  Using Editor.commit() instead.");
            }
            return null;
        }
    }
    ...
    public static void commitOrApply(Editor editor) {
        if (APPLY_METHOD != null) {
            try {
                ...
                if (DEBUG) {
                    L.d(TAG, "Failed while using Editor.apply().  Using Editor.commit() instead.");
                }
